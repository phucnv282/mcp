...
API_KEY = environ.get("ALPHA_VANTAGE_API_KEY", "xxx")  # Get from environment or use default
...
class AlphaVantageAPI:
    ...
    def get_intraday_data(symbol: str, interval: str = "1min", outputsize: str = "compact") -> pd.DataFrame:
        ...
        try:
            ...
            raise ConnectionError(f"Failed to connect to AlphaVantage API: {e}")
        except ValueError:
...
def get_market_data(symbol: str, interval: str = "1min", force_refresh: bool = False) -> pd.DataFrame:
    """
    Get market data from cache or fetch fresh data
    ...
    if (cache_key not in market_data_cache or 
            force_refresh or 
            (current_time - market_data_cache[cache_key].last_updated).seconds > 300):  # Data older than 5 minutes
        df = AlphaVantageAPI.get_intraday_data(symbol, interval, outputsize="full")
...
def calculate_moving_averages(symbol: str, short_period: int = 20, long_period: int = 50) -> Dict[str, Any]:
    ...
    """
    data = get_market_data(symbol, "1min")
...
def calculate_rsi(symbol: str, period: int = 14) -> Dict[str, Any]:
    ...
    """
    data = get_market_data(symbol, "1min").copy()
